...
base_url = 'https://earthexplorer.usgs.gov/inventory/json/v/1.4.1/{context}'
mock_login_url = base_url.format(context='login')
mock_logout_url = base_url.format(context='logout')
mock_search_url = base_url.format(context='search')
mock_download_url = 'https://earthexplorer.usgs.gov/download/5e83d0b84df8d8c2/LC82250672020002LGN00/EE/'
...
class TestUSGS:
    ...
    def test_download(self, mock_ee, app, requests_mock, catalog_usgs):
        ...
        requests_mock.get(mock_search_url, json={'error': [], 'data': dict(results=catalog_usgs)}, status_code=200,
                          headers={'content-type': 'application/json'})
        ...
        # Mock logout url since it is attached to destructor
        requests_mock.get(mock_logout_url, json={'error': [], 'data': {}}, status_code=200,
                          headers={'content-type': 'application/json'})
